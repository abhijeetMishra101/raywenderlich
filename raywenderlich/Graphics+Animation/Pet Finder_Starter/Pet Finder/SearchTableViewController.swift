...
class SearchTableViewController: UITableViewController, UITextFieldDelegate {
  ...
  override func viewDidLoad() {
    ...
    navigationItem.leftBarButtonItem = UIBarButtonItem(title: "Cancel", style: UIBarButtonItemStyle.done, target: self, action: #selector(SearchTableViewController.dismiss as (SearchTableViewController) -> () -> ()))
    
    updateNumberOfPaws(pawStepper)
    updateDistance(distanceSlider)
    
    speciesSelector.setImage(UIImage.init(named: "dog"), forSegmentAt: 0)
    speciesSelector.setImage(UIImage.init(named: "cat"), forSegmentAt: 1)
  }
